...
class _LoginPageState extends State<LoginPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: SafeArea(
          child: Container(
              padding: EdgeInsets.only(top: 32, right: 26, left: 26),
              child: Column(
                children: <Widget>[
                  Row(
                    children: <Widget>[
                                  ...
                                  child: available
                                    ...
                                    onTap: (_seconds == countdown)
                                      ...
                                      if (phoneController.text.isEmpty) {
                                        Toast.show('手机号码不能为空', context);
                                        return;
                                      } else {
                                        FocusScope.of(context).requestFocus(FocusNode());
                                        _sendSSM(phoneController.text);
                                      }
                                    }
                                        : null,
                                  )
                      ...
                      Center(
                        child: Padding(
                            ...
                            child: GestureDetector(
                              onTap: () {
                                FocusScope.of(context).requestFocus(FocusNode());
                                _loginInfo(phoneController.text,codeController.text);
                              },
                              child: Container(
                                color: ColorUtils.appLoginBacColor,
                                child: Padding(
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: <Widget>[
                        ...
                        Text('登陆即代表您同意', style: TextStyle(
                            color: ColorUtils.appHomePagingTextColor,
                            fontSize: 12),),
                        Text('《鱼儿飞用户注册协议》', style: TextStyle(
  ...
  _sendSSM(String userPhone) async {
    try {
      ...
      await HttpUtil.getInstance().post(
          CommonCode.POST_GET_PHONE_SMS, data: {"Phone": userPhone});
      if (res is String) {
 ...
 _loginInfo(String userPhone,String userSsmCode) async{
   ...
   try {
     ...
     final res =
         await HttpUtil.getInstance().post(
         CommonCode.POST_ADMIN_LOGIN, data: {"Phone": userPhone,"PhoneCode":userSsmCode});
     if (res is String) {
       resJson = json.decode(res);
     } else if (res is Map<String, dynamic>) {
       resJson = res;
